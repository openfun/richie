...
export const CourseGlimpse = ({ course }: CourseGlimpseProps) => {
  const relevantCategory = course.categories_data.filter(
    categoryData =>
      categoryData.placeholders.filter(
        placeholder =>
          placeholder.slot === 'course_icons' && placeholder.position === 0,
      ).length,
  )[0];
      ...
      {relevantCategory ? (
        <div className="course-glimpse__icon">
          <div
            ...
            style={{ background: relevantCategory.color || undefined }}
          >
            {relevantCategory.title}
            <div
              className="course-glimpse__icon__band__end"
              style={{ borderLeftColor: relevantCategory.color || undefined }}
            ></div>
          </div>
          {relevantCategory.icon ? (
            <img
              src={relevantCategory.icon.src}
              srcSet={relevantCategory.icon.srcset}
              sizes={relevantCategory.icon.sizes}
              alt=""
            />
          ) : null}
        </div>

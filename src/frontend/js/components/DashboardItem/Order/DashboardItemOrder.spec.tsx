...
import { DATETIME_FORMAT, DEFAULT_DATE_FORMAT } from 'utils/useDateFormat';
...
describe('<DashboardItemOrder/>', () => {
  ...
  it('crashes if no course is provided', async () => {
    ...
    expect(() => render(<DashboardItemOrder order={order} />)).toThrow(
      'Order must provide course attribute',
    );
  ...
  it('renders a order with certificate', async () => {
  ...
  it('renders a writable order with not enrolled target course and enrolls it', async () => {
    ...
    (confirm as Mock).mockReturnValue(true);

    expect(confirm).not.toHaveBeenCalled();
    fireEvent.click(enrollButton);

    expect(confirm).toHaveBeenCalledWith('Are you sure that you want to enroll in this session?');
  ...
  it('renders a writable order with not enrolled target course and refuse the confirm message when enrolling', async () => {
    // Initial order without enrollment.
    const order: Order = {
      ...OrderFactory.generate(),
      target_courses: [TargetCourseFactory.generate()],
    ...
    (confirm as Mock).mockReturnValue(false);
    ...
    // Make sure the courseRun is still not enrolled.
    runElement = await screen.findByTestId(
      'dashboard-item__course-enrolling__run__' + courseRun.id,
    );
    getByRole(runElement, 'button', { name: 'Enroll' });
  ...
  it('renders a writable order with enrolled target course and changes the enrollment', async () => {
    ...
    expect(confirm).not.toHaveBeenCalled();
    fireEvent.click(enrollButton);
    expect(confirm).toHaveBeenCalledWith('Are you sure you want to change your session?');

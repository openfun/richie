...
def walk_api_json_list(root_url,
                       page_length_arg='rpp',
                       page_length=50,
                       paginate_arg='page',
    ...
    while total_count > offset:
        response = requests.get(
            root_url,
            params={
                paginate_arg: offset if paginate_type == 'offset' else 1 + offset // page_length,
        ...
        try:
            ...
            raise ApiConsumingException(
                'HTTP Request during API walk failed with code {:n}'.format(response.status_code),
            )
        ...
        try:
            ...
            raise ApiConsumingException(
                'Cannot read total_count_key {:s} in json response'.format(total_count_key),
            )

...
def regenerate_indexes(logger):
    ...
    indexes_to_create = zip(
        list(map(lambda ix: perform_create_index(ix, logger), indexables)),
        indexables,
    )

    # Prepare to alias them so they can be swapped-in for the previous versions
    actions_to_create_aliases = [
        {'add': {'index': index, 'alias': ix.index_name}} for index, ix in indexes_to_create
    ]
    ...
    indices_client.update_aliases(dict(

...
class CreateCmsDataTests(CMSTestCase):
    ...
    def test_command_create_cms_data(self, mock_page, mock_organization, mock_organization_page):
        """
        ...
        expected_calls_for_root_pages = [
            (({'en': 'Home', 'fr': 'Accueil'}, ), {
                'in_navigation': True, 'is_homepage': True, 'published': True, 'site': site,
                'template': 'richie/fullwidth.html'
            }),
            (({'en': 'News', 'fr': 'Actualités'}, ), {
                'in_navigation': True, 'is_homepage': False, 'published': True, 'site': site,
                'template': 'richie/fullwidth.html'
            }),
        ...
        for i, actual_call in enumerate(mock_page.call_args_list[6:]):
            expected_call = (
                ({'en': 'Organization #{:d}'.format(i), 'fr': 'Organisation #{:d}'.format(i)},), {
                    'parent': 3, 'published': True, 'site': site,
